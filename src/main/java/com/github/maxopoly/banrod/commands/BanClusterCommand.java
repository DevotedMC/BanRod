...
@ZCommand(description = "Bans a Cluster", altIds = "bc", id = "banCluster", minArgs = 1, maxArgs = 3)
public class BanClusterCommand extends ZeusCommand {
    @Override
    public String handle(CommandSender sender, String command) {
        ...
        BanRodPlugin.getInstance().getAccountBanManager().banCluster(targetPlayer, now, now.plusMillis(howLong), comment, "ZConsole", BanEffect.CLUSTER_BAN);
        return "Successfully banned cluster of: " + targetPlayer;
    }
}
